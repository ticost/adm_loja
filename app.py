...
def get_db_connection():
    ...
    try:
        ...
        if "planetscale" not in st.secrets:
            ...
            st.info("💡 Vá em 'Manage app' → 'Settings' → 'Secrets' para configurar")
            return None
...
def init_auth_db():
    ...
    if not conn:
        st.warning("⚠️ Não foi possível conectar ao banco. Verifique a configuração dos Secrets.")
        return
...
# ... (AS DEMAIS FUNÇÕES PERMANECEM IGUAIS - mantive o código original para brevidade)
...
# =============================================================================
# PÁGINA DE CONFIGURAÇÃO (APARECE SE SECRETS NÃO ESTIVEREM CONFIGURADOS)
# =============================================================================
...
if "planetscale" not in st.secrets:
    ...
    Para usar esta aplicação, você precisa configurar as credenciais do banco de dados.
    """)
    ...
    2. Clique em **'Manage app'** no canto inferior direito
    3. Vá em **Settings** → **Secrets**
    4. Cole o seguinte formato:
    ```toml
    [planetscale]
    host = "aws.connect.psdb.cloud"
    user = "seu-usuario-do-planetscale"
    password = "sua-senha-do-planetscale" 
    database = "nome-do-seu-database"
    ```
    5. Clique em **Save**
    6. A aplicação reiniciará automaticamente
    
    ### 🔑 Onde obter as credenciais:
    - Acesse sua conta no **PlanetScale**
    - Vá no seu banco de dados
    - Clique em **"Connect"**
    - Selecione **"PyMySQL"**
...
# =============================================================================
# INICIALIZAR BANCOS DE DADOS (APÓS VERIFICAÇÃO DOS SECRETS)
# =============================================================================

# Inicializar bancos de dados apenas se secrets estiverem configurados
try:
    init_db()
    init_auth_db()
except Exception as e:
...
if not st.session_state.logged_in:
    ...
    if not conn:
        ...
        Verifique:
        - Se os **Secrets** estão corretos
        - Se as **credenciais** do PlanetScale estão válidas
...
# ... (O RESTO DO CÓDIGO DA APLICAÇÃO PRINCIPAL PERMANECE IGUAL)
