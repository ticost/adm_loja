...
def gerar_html_agenda_contatos(users):
    ...
    <head>
        ...
        <title>Agenda de Contatos - Sistema Livro Caixa</title>
        <style>
            body {{
    ...
    <body>
        <div class="header">
            ...
            <div class="subtitle">
                Sistema Livro Caixa | Gerado em: {datetime.now().strftime('%d/%m/%Y às %H:%M')}
            </div>
        </div>
    """
    ...
    html_content += f"""
        <div class="footer">
            Total de contatos: {len(users)} | Sistema Livro Caixa © {datetime.now().year}
        </div>
    </body>
    </html>
    """
...
def visualizar_agenda_contatos():
    ...
    if not user_is_admin():
        st.warning("⚠️ Apenas administradores podem acessar a agenda de contatos")
        return
    ...
    for user in users:
        ...
        if busca:
            ...
            match_busca = (
                (nome_completo and busca_lower in nome_completo.lower()) or
        ...
        if filtro_permissao != "Todos":
            if PERMISSOES.get(permissao) != filtro_permissao:
                continue
...
def show_main_application():
    ...
    elif selected_menu == "📒 Agenda de Contatos" and user_is_admin():
